import { BACK_BALL_SIZE, BACK_WIN_HEIGHT, PADDLE_HEIGHT, PADDLE_WIDTH, STARTINGPOS_LEFT_X, STARTINGPOS_RIGHT_X } from "./constants";
...
function paddleBounceLeft(ball: any, leftPosY: number, height: number) {
	...
	export function handleWallBounce(ball: any, leftPlayer : any, rightPlayer :any, powerup: any) {
		...
		if(ball.pos.x + BACK_BALL_SIZE > STARTINGPOS_RIGHT_X && ball.pos.x + BACK_BALL_SIZE < STARTINGPOS_RIGHT_X + (PADDLE_WIDTH / 2) && //check si la balle depasse au maximum de la moitie de la width
			ball.pos.y > rightPosY - rightHeight / 2 && ball.pos.y < rightPosY + rightHeight / 2 &&
			...
			ball.pos.y > leftPosY - leftHeight / 2 && ball.pos.y < leftPosY + leftHeight / 2 &&
