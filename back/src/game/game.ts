...
function handleWallBounce(ball: any, leftPosY: number, rightPosY: number) {
  if (
    ...
    ball.pos.y > rightPosY - PADDLE_HEIGHT / 2 &&
    ball.pos.y < rightPosY + PADDLE_HEIGHT / 2 &&
    ball.angle < 180
  ) {
    //hit right
    ...
    ball.pos.y > leftPosY - PADDLE_HEIGHT / 2 &&
    ball.pos.y < leftPosY + PADDLE_HEIGHT / 2 &&
    ball.angle > 180
  ) {
    //hit left
...
function resetBall(side: number) {
  ...
  let angle = 0;
  if (side === 0) angle = getRandomArbitrary(20, 160);
  else if (side === 1) angle = getRandomArbitrary(200, 340);
  else if (side === 2)
    angle =
      today.getMilliseconds() % 2
        ? getRandomArbitrary(20, 160)
        : getRandomArbitrary(200, 340);
  const ball = {
    pos: {
      x: BACK_WIN_WIDTH / 2,
      y: BACK_WIN_HEIGHT / 2,
