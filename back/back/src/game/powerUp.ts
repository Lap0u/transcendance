...
function placePowerup(powerup: any) {
  ...
  powerup.status = 1;
  powerup.pos.x = Math.round(
    getRandomArbitrary(POWERUPMARGIN, BACK_WIN_WIDTH - POWERUPMARGIN),
  );
  powerup.pos.y = Math.round(
    getRandomArbitrary(POWERUPMARGIN / 2, BACK_WIN_HEIGHT - POWERUPMARGIN / 2),
  );
}
...
function handleDelay(powerup: any) {
  if (powerup.status === 0) {
    powerup.delay--;
    if (powerup.delay === 0) placePowerup(powerup);
  } else if (powerup.status === 2) {
    powerup.delay--;
    if (powerup.delay < 0) resetPowerup(powerup);
  }
}
...
function playerCaughtPowerup(ballAngle: number, powerup: any) {
  ...
  if (powerup.value === 3) powerup.delay *= 2;
}
...
export function handlePowerUp(ball: any, powerup: any, settings: any) {
  if (settings && settings.powerup === true) {
    handleDelay(powerup);
    if (powerup.status === 1) handlePickUp(ball, powerup);
  }
}
