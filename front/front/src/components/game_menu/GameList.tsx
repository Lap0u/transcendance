...
const OneGame = ({ game, customGameValues }: singleGamesprops) => {
  ...
  return (
    <div className="one-game-line">
      <Button onClick={joinGame} type="link">
        Spectate{' '}
      </Button>
      <li className="one-game-list">
        <span className="leftPlayer">{game.playerOne.accountUsername}</span>
        vs
        <span className="rightPlayer">{game.playerTwo.accountUsername}</span>
      </li>
    </div>
  );
};
...
export const GameList = ({ games, customGameValues }: gameslistprops) => {
  ...
  const listItems = games.map((d) => (
    <OneGame key={d.gameId} game={d} customGameValues={customGameValues} />
  ));
  return (
