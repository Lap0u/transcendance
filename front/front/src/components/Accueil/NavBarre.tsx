...
function NavigationBarre({
      ...
      label: (
        <div className="info">
          ...
          <FriendListPopover
            currentUser={user}
            isVisible={isVisible}
            setIsVisible={setIsVisible}></FriendListPopover>
  ...
  return isLoginActive ? (
    <Menu
      items={menuItems}
      style={{ width: '100vw', border: 'none', position: 'relative' }}
      mode="horizontal"
      className="nav-barre"></Menu>
  ) : null;
}
...
function FriendList({ currentUser }: { currentUser: TypeUserDto }) {
  ...
  return ok ? (
    <div className="friends">
      ...
      {friendList.length === 0 ? <p>No friends yet</p> : null}
    </div>
  ) : null;
}
function FriendListPopover({
  currentUser,
  isVisible,
  setIsVisible,
}: {
  currentUser: TypeUserDto;
  isVisible: boolean;
  setIsVisible: any;
}) {
  return (
    <Popover
      ...
      <div className="onglet-nav">Friend List</div>
    </Popover>
  );
}
