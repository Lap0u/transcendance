...
export function TwoAuthAutenticatePage({
    ...
    await axios
      ...
      .then((res) => {
        if (res.data.status === 401) {
          ...
          message.success(
            'Your account has been verified, proceed to the signin page'
          );
  ...
  return (
    <div className="two-auth">
      <h2 className="two-auth-title">
        Your have to validate your authentification
      </h2>
      <div className="card-body">
        <form onSubmit={(e) => codeForm(e, changeUser, userChanged)} id="form">
          <div className="two-auth-info">
            Please provide the code your reseived on your email adress.
            <br />
            <input
        ...
        <div className="resend">
          <i>
            If you did not receive any code click here to resend mail (this
            might take few minuts to receive the mail, don't forget to check
            your spam){' '}
          </i>
          <button
            className="btn btn--radius-2 btn--blue-2"
            type="submit"
            onClick={() => resend(changeUser, userChanged)}>
            Resend
          </button>
        </div>
      </div>
